...
| Megatron-LM | Custom script | Automatic | Reads SLURM vars directly |
...
For detailed explanations of each framework's concepts and internals, refer to the earlier chapters: DDP in Chapter~\ref{chap:distributed-training-with-pytorch-ddp}, FSDP in Chapter~\ref{chap:fsdp-memory-efficient-distributed-training}, and DeepSpeed in Chapter~\ref{chap:deepspeed-zero-and-advanced-optimization}. Here we focus specifically on the SLURM launch patterns.

### DDP with SLURM {#sec:slurm-ddp-example}

DDP replicates the model on each GPU and synchronizes gradients via AllReduce. With SLURM, you launch DDP using `torchrun` wrapped by `srun`. The key is translating SLURM's environment variables to what `torchrun` expects.
...
FSDP shards model parameters across GPUs, reducing per-GPU memory requirements. The SLURM launch pattern is identical to DDP—you use `torchrun` the same way. The difference is in the Python code where you wrap the model with `FullyShardedDataParallel` instead of `DistributedDataParallel`.
...
Note the higher `--cpus-per-task` and `--mem` allocations—FSDP's CPU offloading benefits from more CPU resources.

### DeepSpeed with SLURM {#sec:slurm-deepspeed-example}

DeepSpeed differs from DDP/FSDP in a key way: it handles distributed initialization internally. You don't need `torchrun`—just run Python directly, and DeepSpeed reads SLURM environment variables automatically.

Training script structure (full version in `code/deepspeed/train.py`):
...
def main():
    deepspeed.init_distributed()  # Reads SLURM vars automatically
    ...
    # Training loop using model_engine.backward() and model_engine.step()
...
SLURM batch script (full version in `code/deepspeed/run.slurm`):
...
Notice how DeepSpeed's SLURM integration differs from DDP and FSDP. There's no `torchrun` wrapper—you run `python` directly because DeepSpeed handles process group initialization internally through `deepspeed.init_distributed()`. The batch script still exports `RANK`, `LOCAL_RANK`, and `WORLD_SIZE` from SLURM variables, but DeepSpeed reads these automatically rather than requiring you to pass them through a launcher. Another distinction is that training configuration lives in a JSON file rather than being hardcoded—this separation makes it easy to experiment with different ZeRO stages, offloading strategies, and optimizer settings without modifying your training script.

### Megatron-LM with SLURM {#sec:slurm-megatron-example}

Megatron-LM is designed for the largest models requiring multi-dimensional parallelism (tensor, pipeline, data). It has its own launch infrastructure that integrates with SLURM.

**Prerequisites**: Install Megatron-LM from source (the PyPI `megatron-core` package doesn't include `megatron.training`):
...
SLURM batch script (full version in `code/megatron/run.slurm`):
...
PP=2  # Pipeline parallelism (across 2 nodes)
...
Megatron-LM's parallelism dimensions must satisfy: `TP × PP × DP = total GPUs`. In this example with 2 GPUs total, we use `PP=2` (pipeline across nodes) with `TP=1` and `DP=1`.

**Checkpoint management**: Megatron-LM saves checkpoints in a distributed format (`.distcp` files) for efficient parallel I/O. For an 8B model, expect ~108 GB checkpoint size (16 GB bf16 weights + 64 GB fp32 optimizer states + overhead). To convert checkpoints for use with other frameworks:
...
python code/megatron/convert_megatron_checkpoint.py \
    ...
    --vocab-size 128256 --bf16
```

The exported checkpoint is standalone—no Megatron required. For HuggingFace format, consider [Megatron-Bridge](https://github.com/NVIDIA-NeMo/Megatron-Bridge). See `code/megatron/` for complete examples.
