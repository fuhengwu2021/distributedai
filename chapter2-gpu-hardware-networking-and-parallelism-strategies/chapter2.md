...
[^computational-gap-data]: GPU line (blue solid): mainstream single-GPU HBM by deployment year—A100 40GB (2020), A100 80GB (2021), H100 80GB (2022–2023), H200 141GB (2024), B200 192GB (2025–2026). Yellow dashed (disclosed): one milestone per release year from @tbl:model-comparison (entries without ~) plus MT-NLG (2021) and PaLM (2022)—each year picks the smallest disclosed model larger than the prior year's choice; if none qualify, the year's sole disclosed option is used (2024): GPT-3 175B (~350GB), MT-NLG 530B (~1060GB), PaLM 540B (~1080GB), PanGu-Σ 1.085T (~2170GB), DeepSeek-V2 236B (~472GB), DeepSeek-V3 671B (~1342GB), Kimi K2.6 1T (~2000GB). Red solid (frontier): largest model per release year in @tbl:model-comparison including approximate (~) entries—GPT-3 175B, MT-NLG 530B, PaLM 540B, Gemini-1 1.6T (~3200GB), GPT-4V ~1.8T (~3600GB), GPT-5 ~2T (~4000GB; low bound of ~2–5T), Claude Mythos 5 ~10T (~20000GB). All model values are BF16 weights (2 bytes/param). Values shown are model weights only; training also requires gradients, optimizer states, and activations, further necessitating distributed training.
